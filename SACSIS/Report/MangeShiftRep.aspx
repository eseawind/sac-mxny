<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MangeShiftRep.aspx.cs"
    Inherits="SACSIS.Report.MangeShiftRep" %>
    ...
    <script type="text/javascript">
        ...
        function DellAttribute(key) {
            ...
            $.messager.confirm('删除报表', '你确定要删除该条数据吗?', function (ok) {
                if (ok) {
                    $.post("MangeShiftRep.aspx", { param: 'Attribute', key: key
                    }, function (data) {
        ...
        function RadioChange() {
            $("#dv_radio_eidt").change(function () {
                ...
                $.post("MangeShiftRep.aspx", { param: 'name', id: sign }, function (data) {
        ...
        function EditTableSunInfo(ID_KEY) {
            ...
            $.post("MangeShiftRep.aspx", { param: 'sunEdit', name1: escape($("#txtName1_edit").val()), jz: $("#txtJz_edit").val(), order: $("#txtOrder_edit").val(), unit: escape($("#txtUnit_edit").val()),
        ...
        function EditRadio(id) {
            $.post("MangeShiftRep.aspx", { param: 'pointEdit' }, function (data) {
                var list = data.list;
                if (list != null) {
        ...
        function addSun() {
            ...
            $.post("MangeShiftRep.aspx", { param: 'addSun', tid: escape($("#tbID").val()), tName: escape($("#tbName").val()), id: escape(pid)
            }, function (data) {
                if (data.count == "1") {
        ...
        //初始化单选框 复选框
        function InitRadio() {
            $.post("MangeShiftRep.aspx", { param: 'point' }, function (data) {
                var list = data.list;
                if (list != null) {
                    var htmlb = '';
                    //绑定风场数据
        ...
        function Dell(id) {
            $.messager.confirm('删除报表', '你确定要删除该报表吗?', function (ok) {
                if (ok) {
                    $.post("MangeShiftRep.aspx", { param: 'dellTable', id: id }, function (data) {
                        if (data.count == "1") {
        ...
        function editTableInfo(id, name) {
            $.post("MangeShiftRep.aspx", { param: 'edittable', nameo: escape(name), ido: escape(id), name: escape($("#txttableName").val()), id: escape($("#txttableID").val()) }, function (data) {
                if (data.count == "1") {
                    $.messager.alert('编辑报表!', '报表编辑成功!', 'info');
                    showList();
        ...
        //添加报表名称
        function add() {
            $.post("MangeShiftRep.aspx", { param: 'add', name: escape($("#txtTableName").val()), id: escape($("#txtId").val()), model: $("#mType").val() }, function (data) {
                if (data.count == "1") {
