<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ManageChartPara.aspx.cs"
    Inherits="SACSIS.Report.ManageChartPara" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function InitTable() {
            $('#tableList').datagrid({
                ...
                columns: [[
				    ...
				    { field: 'X', title: 'X轴指标', width: 220, align: 'center' },
                    { field: 'Y', title: 'Y轴指标', width: 220, align: 'center' },
					{ field: 'optEdit', title: '编 辑', width: 80, align: 'center',
					    formatter: function (value, rec, index) {
					        var up = '';
					        up = '<a href="javascript:void(0);" mce_href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:icon-search" onclick="EditPoint(\'' + rec.ID + '\',\'' + rec.X + '\',\''
                            + rec.Y + '\')" data-options="iconCls:icon-search">编 辑</a>';
					        return up;
					    }
					},
        ...
        function EditPoints(key) {
            $.post("ManageChartPara.aspx", { param: 'edit', key: key, x: escape($("#txtX_edit").val()), y: escape($("#txtY_edit").val())
            }, function (data) {
                if (data.count == "1") {
                    $('#dv_point_edit').dialog('close');
                    $.messager.alert('拟合趋势关系!', '关系编辑成功!', 'info');
                    showList();
                }
                else
                    $.messager.alert('拟合趋势关系!', '关系编辑失败!', 'info');
            }, 'json');
        }
        ...
        //删除测点
        function Dell(id) {
            $.messager.confirm('删除关系', '你确定要删除该条数据吗?', function (ok) {
                if (ok) {
                    $.post("ManageChartPara.aspx", { param: 'dell', id: id }, function (data) {
                        if (data.count == "1") {
                            $.messager.alert('拟合趋势关系!', '关系删除成功!', 'info');
                            showList();
                        }
                        else
                            $.messager.alert('拟合趋势关系!', '关系删除失败!', 'info');
                    }, 'json');
                } else {
                    $.messager.alert('拟合趋势关系', '删除已取消!', 'info');
                }
            });
        }
        ...
        //添加测点
        function add() {
            $.post("ManageChartPara.aspx", { param: 'add', x: escape($("#txtX").val()), y: escape($("#txtY").val())
            }, function (data) {
                if (data.count == "1") {
                    $.messager.alert('拟合趋势关系!', '关系添加成功!', 'info');
                    showList();
                    $('#dv_points').dialog('close');
                }
                else
                    $.messager.alert('拟合趋势关系!', '关系添加失败!', 'info');
            }, 'json');
