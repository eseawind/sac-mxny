<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ManagePoint.aspx.cs" Inherits="SACSIS.Report.ManagePoint" %>
    ...
    <script type="text/javascript">
        ...
        function InitTable() {
            $('#tableList').datagrid({
                ...
                idField: 'ID',
                columns: [[
                    { field: 'KEY', title: 'KEY', width: 60, align: 'center' },
				    { field: 'ID', title: '编号', width: 60, align: 'center', hidden: true },
                    { field: 'T_UNITID', title: '机组', width: 60, align: 'center' },
                    { field: 'T_POWERTAG', title: '负荷', width: 160, align: 'center' },
                    { field: 'T_FLOWTAG', title: '主汽流量', width: 160, align: 'center' },
                    { field: 'T_PRESSURETAG', title: '主汽压力', width: 160, align: 'center' },
                    { field: 'T_TEMPERATURETAG', title: '主汽温度', width: 160, align: 'center' },
                    { field: 'T_REHEATTEMPERATURETAG', title: '再热温度', width: 160, align: 'center' },
                    { field: 'T_VACUUM', title: '真空', width: 160, align: 'center' },
                    { field: 'T_EFFICIENCY', title: '锅炉效率', width: 160, align: 'center' },
                    { field: 'T_HEATCONSUMPTION', title: '热耗', width: 160, align: 'center' },
                    { field: 'T_COALCONSUMPTION', title: '煤耗', width: 160, align: 'center' },
                    { field: 'T_INFO_RL', title: '负荷指标', width: 160, align: 'center', hidden: true },
					{ field: 'optEdit', title: '编 辑', width: 80, align: 'center',
					    formatter: function (value, rec, index) {
					        var up = '';
					        //					        up = '<a href="javascript:void(0);" mce_href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:icon-search" onclick="EditPoint(\'' + rec.ID + '\',\''
					        //                            + rec.T_UNITID + '\',\'' + rec.T_POWERTAG + '\',\'' + rec.T_FLOWTAG + '\',\'' + rec.T_PRESSURETAG + '\',\'' + rec.T_TEMPERATURETAG + '\',\'' + rec.T_REHEATTEMPERATURETAG + '\',\'' + rec.T_VACUUM + '\',\''
					        //                           + rec.T_EFFICIENCY + '\',\'' + rec.T_HEATCONSUMPTION + '\',\'' + rec.T_COALCONSUMPTION + '\',\'' + rec.T_INFO_RL + '\')" data-options="iconCls:icon-search">编 辑</a>';
					        return up;
					    }
					},
        ...
        function EditPoints(key) {
            ...
            $.post("ManagePoint.aspx", { param: 'edit', key: key, paraid: escape($("#txtPointID_edit").val()), desc: escape($("#txtPointDesc_edit").val()),
        ...
        function Dell(id) {
            $.messager.confirm('删除测点', '你确定要删除该测点吗?', function (ok) {
                if (ok) {
                    $.post("ManagePoint.aspx", { param: 'dellPoint', id: id }, function (data) {
                        if (data.count == "1") {
        ...
        function add() {
            ...
            $.post("ManagePoint.aspx", { param: 'add', unit: escape($("#txtUnit").val()), power: escape($("#txtPower").val()), flow: escape($("#txtFlow").val()),
                yl: escape($("#txtYl").val()), zqwd: escape($("#txtZQWD").val()), zrwd: escape($("#txtZRWD").val()), zk: $("#txtZK").val(), glxl: escape($("#txtgLXL").val()),
                rh: escape($("#txtRH").val()), mh: escape($("#txtMH").val())
            }, function (data) {
                if (data.count == "1") {
...
<body>
    ...
    <div id="dv_points" style="display: none; width: 500px; overflow-y: auto; height: 220px;">
